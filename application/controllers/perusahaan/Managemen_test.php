 ...
 * 
 */
class Managemen_test extends CI_Controller
{
    ...
    public function tambah(){
        ...
        if ($this->form_validation->run() == FALSE) {
            getViews($data, 'v_company/v_add_seleksi');
        } else {
            //insert tes seleksi
            $data = [
            ...
            if ($insert > 0) {
                ...
                redirect('perusahaan/managemen_test');
            } else {
    ...
    public function perbarui($id)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            getViews($data, 'v_company/v_edit_seleksi');
        } else {
            //update data
            $data = [
                'nama_tes_seleksi' => $this->input->post('nama', true),
    ...
    public function detail($id)
    {
        ...
        getViews($data, 'v_company/v_detail_seleksi');
    }
    ...
    public function konfigurasi(){
        ...
        if ($this->form_validation->run() == FALSE) {
            getViews($data, 'v_company/v_konfigurasi_tes');
        }else{
            ...
            if ($insert) {
                ...
                redirect('perusahaan/managemen_test/konfigurasi');
            }else{
    ...
    public function tambah_soal($id)
    {
        ...
        getViews($data, 'v_company/v_add_soal_seleksi');
        ...
        if (!empty($soal_cek)) {
            ...
            $this->session->set_flashdata('msg_success', 'Selamat, Data Seleksi Berhasil Ditambahkan');
            redirect('perusahaan/managemen_test/detail/' . $id_seleksi);
        }
    }
    ...
    public function update_soal($id)
    {
        if (isset($_POST['id']) && !empty($_POST['id'])) {
            ...
            if ($update) {
                $this->session->set_flashdata('msg_success', 'Selamat, Data Seleksi Berhasil Ditambahkan');
                redirect('perusahaan/managemen_test/detail/' . $id);
            } else {
                $this->session->set_flashdata('msg_success', 'Selamat, Data Seleksi Berhasil Ditambahkan');
                redirect('perusahaan/managemen_test/detail/' . $id);
            }
