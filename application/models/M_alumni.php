...
{ 
	...
	public function getLowongan($bulan){
		return $this->db->query("SELECT * FROM `lowongan` JOIN company ON company.id_company=lowongan.id_company WHERE lowongan.status = 'verifikasi' AND MONTH(lowongan.create_at) =".$bulan)->result_array();
	...
	public function getDetailAlumni($nisn){
		return $this->db->query("SELECT alumni.*, status_alumni.*, jurusan.nama_jurusan FROM `alumni` LEFT JOIN jurusan ON jurusan.id_jurusan=alumni.id_jurusan LEFT JOIN status_alumni ON status_alumni.nisn=alumni.nisn WHERE alumni.nisn = $nisn")->row_array();
	}
