...
class M_home extends CI_Model
{
    ...
    {
        return $this->db->query("SELECT * FROM `lowongan` JOIN company ON company.id_company=lowongan.id_company WHERE lowongan.status = 'verifikasi' ORDER BY lowongan.create_at DESC LIMIT 4")->result_array();
    }
    ...
    public function getLowonganAll(){
        return $this->db->query("SELECT * FROM `lowongan` JOIN company ON company.id_company=lowongan.id_company WHERE lowongan.status = 'verifikasi' ORDER BY lowongan.create_at DESC")->result_array();
    }
